...
public class CategoryHandler(AppDbContext context) : ICategoryHandler
{
    public async Task<Response<Category?>> CreateAsync(CreateCategoryRequest request)
    {
        try {
            ...
            return new Response<Category?>(category, 201, "Cateroria criada com sucesso!");
        } 
        catch 
        {
    ...
    public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
    {
        ...
        {
            return new PagedResponse<List<Category>?>(null, 404, "Nenhuma categoria encontrada.");
        }
    }
    ...
    public async  Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
    {
        try
        {
            ...
            return category is null ? new Response<Category?>(category) :  new Response<Category?>(null, 404, "Categoria não encontrada!");
        } catch
        {
            return new Response<Category?>(null, 500, "Não foi possível excluir a categoria!");
        }
